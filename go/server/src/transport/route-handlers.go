...
func GetDefaultCity(context *gin.Context) {
	idx := algorithm.LinearSearch(city.Cities, func(city city.City) bool {
		return city.Name == "Нижний Тагил"
	})
	context.IndentedJSON(http.StatusOK, city.Cities[idx])
}

// GetListCities
// TODO change on regis in future
func GetListCities(context *gin.Context) {
	context.IndentedJSON(http.StatusOK, city.Cities)
}
